    ...
    <div className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
      ...
      {comments.length ? <ol className="mt-6 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {comments.map(comment => {
          const commentPath = `${albumPath(comment.album)}#comment-${comment.id}`;
          return <li key={comment.id} className="flex min-w-0 flex-col rounded-2xl border border-zinc-100 p-4">
            ...
            <div className="mt-4 grid grid-cols-[4rem_minmax(0,1fr)] items-center gap-3">
              <Link href={commentPath}><Artwork src={comment.album.cover_url} alt={`Okładka albumu ${comment.album.title}`} /></Link>
              <div className="min-w-0">
                <Link href={commentPath} className="block truncate font-black hover:underline">{comment.album.title}</Link>
                {comment.rating !== null ? <div className="mt-1">
