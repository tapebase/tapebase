...
export type NotificationView = {
  id: number;
  kind: "submission_approved" | "submission_imported" | "submission_rejected" | "comment_reply" | "comment_like";
  message: string;
  read_at: string | null;
  created_at: string;
  href: string;
...
export function NotificationsPanel({ notifications }: { notifications: NotificationView[] }) {
  ...
  return <section className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-3xl font-black">Powiadomienia</h1><p className="mt-1 text-sm text-zinc-500">Odpowiedzi, polubienia oraz informacje o zgłoszeniach katalogu.</p></div>
    ...
    {notifications.length ? <ul className="mt-6 space-y-3">{notifications.map(item => <li id={`powiadomienie-${item.id}`} key={item.id} className={`rounded-2xl border p-5 ${item.read_at ? "border-zinc-100" : "border-emerald-300 bg-emerald-50"}`}>
      ...
      <Link href={item.href} className="mt-4 inline-flex rounded-xl bg-zinc-950 px-4 py-2 text-sm font-bold text-white">{item.kind === "comment_reply" || item.kind === "comment_like" ? "Otwórz dyskusję" : item.kind === "submission_imported" ? "Otwórz w katalogu" : "Otwórz zgłoszenie"}</Link>
