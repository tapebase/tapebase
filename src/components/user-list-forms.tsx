...
import { addAlbumToUserList, createUserList, deleteUserList, updateUserList } from "@/app/actions/user-lists";
...
export function CreateUserListForm() {
  ...
  return <form action={action} className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
    ...
    <p className="mt-2 text-sm text-zinc-500">Publiczną listę zobaczą inni użytkownicy na Twoim profilu. Prywatna będzie widoczna tylko dla Ciebie.</p>
...
export function EditUserListForm({ id, name, description, isPublic }: { id: number; name: string; description: string | null; isPublic: boolean }) {
...
export function AddAlbumToList({ albumId, lists, returnPath }: { albumId: number; lists: UserListChoice[] | null; returnPath: string }) {
  ...
  return <form action={action} className="mt-4 rounded-2xl bg-white p-4 shadow-sm">
    ...
    <select id="album-list" name="listId" required className="mt-2 w-full rounded-xl border border-zinc-300 bg-white px-3 py-2.5 text-sm">
      {lists.map(list => <option key={list.id} value={list.id}>{list.name}{list.containsAlbum ? " — już dodany" : ""}{list.is_public ? "" : " — prywatna"}</option>)}
    </select>
