...
import { rankingMetadata, topRatedAlbums, topRatedArtists } from "@/lib/catalog";
...
export const metadata = {
  title: "Rankingi",
  description: "Najlepiej oceniani artyści i albumy w społeczności TAPEBASE.",
};
...
export default async function RankingsPage({ searchParams }: RankingsPageProps) {
  ...
  const rankingType = params.typ === "artysci" || params.typ === "albumy" ? params.typ : "wszystkie";
  const showArtists = rankingType !== "albumy";
  const showAlbums = rankingType !== "artysci";
  ...
  return <main className="mx-auto w-full max-w-7xl flex-1 px-6 py-12">
    <header className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
      ...
      <h1 className="mt-2 text-4xl font-black sm:text-5xl">
        {rankingType === "artysci" ? "Najlepiej oceniani artyści" : rankingType === "albumy" ? "Najlepiej oceniane albumy" : "Najlepiej oceniani"}
      </h1>
      ...
      <form action="/rankingi" className="mt-6 grid gap-4 rounded-2xl bg-zinc-50 p-5 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <label className="text-sm font-bold">Rok wydania
          <select name="rok" defaultValue={year} disabled={!showAlbums} className="mt-2 w-full rounded-xl border border-zinc-300 bg-white px-3 py-3 font-normal disabled:opacity-50">
            <option value="">Wszystkie lata</option>
            {metadata.years.map(item => <option key={item}>{item}</option>)}
          </select>
        </label>
        <label className="text-sm font-bold">Rodzaj wydawnictwa
          <select name="rodzaj" defaultValue={releaseType} disabled={!showAlbums} className="mt-2 w-full rounded-xl border border-zinc-300 bg-white px-3 py-3 font-normal disabled:opacity-50">
        ...
        <label className="text-sm font-bold">Gatunek
          <select name="gatunek" defaultValue={genre} disabled={!showAlbums} className="mt-2 w-full rounded-xl border border-zinc-300 bg-white px-3 py-3 font-normal disabled:opacity-50">
      ...
      <p className="mt-3 text-xs text-zinc-500">Filtry roku, rodzaju wydawnictwa i gatunku dotyczą rankingu albumów.</p>
    </header>
