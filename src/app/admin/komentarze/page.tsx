...
export default async function AdminCommentsPage() {
  ...
  const { data, error } = await client.from("comment_reports")
    .select("id,category,details,status,created_at,reporter:users!comment_reports_reporter_id_fkey(username),comment:comments(id,content,hidden_at,hidden_reason,author:users!comments_user_id_fkey(username),album:albums(title,slug),artist:artists(id,name,slug))")
  ...
  return <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-12">
    ...
    <section className="mt-6 rounded-3xl bg-white p-6 shadow-sm sm:p-8">
      {reports.length ? <ol className="space-y-4">{reports.map(report => { const comment = Array.isArray(report.comment) ? report.comment[0] : report.comment; const reporter = Array.isArray(report.reporter) ? report.reporter[0] : report.reporter; if (!comment) return null; const author = Array.isArray(comment.author) ? comment.author[0] : comment.author; const album = Array.isArray(comment.album) ? comment.album[0] : comment.album; const artist = Array.isArray(comment.artist) ? comment.artist[0] : comment.artist; const targetPath = album ? albumPath(album) : artist ? artistPath(artist) : null; return <li key={report.id} className="rounded-2xl border border-zinc-200 p-5">
        <div className="flex flex-wrap justify-between gap-3"><p><strong>{reasonLabel[report.category]}</strong> · zgłosił @{reporter?.username ?? "Użytkownik"} · status: {report.status}</p>{targetPath && <Link href={`${targetPath}#comment-${comment.id}`} className="text-sm font-bold underline">Otwórz komentarz</Link>}</div>
